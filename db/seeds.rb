...
columns_csv = CSV.readlines("db/reviews.csv")
columns_csv.shift
columns_csv.each do |row|
  Columns.create(title: row[1], categori_1: row[2], categori_2: row[3], image: row[4], text: row[5], created_at: row[6], updated_at: row[7])
end